...
import os
from src.sre_copy_objects import *

# Variables for testing DB setup
DB_HOST = '127.0.0.1'
DB_NAME = 'postgres'
...
# Variables for testing s3 environment setup
S3_ENDPOINT_URL = 'http://localhost:9000'
AWS_ACCESS_KEY_ID = 'minioadmin'
AWS_SECRET_ACCESS_KEY = 'minioadmin'
AWS_DEFAULT_REGION = 'us-east-1'

S3_TEST_LEGACY_BUCKET_NAME = 'test-legacy-s3'
S3_TEST_PROD_BUCKET_NAME = 'test-production-s3'
...
TEST_FILENAME = 'tests/paths.txt'
TEST_PATH = 'image/avatar-1.png'
...
class S3ObjectsCopyTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        curs=db_conn.cursor()
        try:
            curs.execute("DROP DATABASE IF EXISTS testing")
            curs.execute("CREATE DATABASE testing")
        except Exception as e:
            logging.error(f"Error while creating testing database: {e}")
            sys.exit(1)
        db_conn.close()
        ...
        try:
            ...
            logging.error(f"Error while connecting to the testing database: {e}")
            sys.exit(1)



        with db_conn.cursor() as curs:
            curs.execute("CREATE TABLE IF NOT EXISTS avatars (id SERIAL PRIMARY KEY,path VARCHAR)")
            db_conn.commit()

        # Create testing S3 buckets
        try:
        ...
        if s3_conn.Bucket(S3_TEST_LEGACY_BUCKET_NAME).creation_date is not None:
            try:
                s3_conn.Bucket(S3_TEST_LEGACY_BUCKET_NAME).objects.all().delete()
                s3_conn.Bucket(S3_TEST_LEGACY_BUCKET_NAME).delete()
                s3_conn.create_bucket(Bucket=S3_TEST_LEGACY_BUCKET_NAME)
            except Exception as e:
                logging.error(f"Error while creating testing buckets: {e}")
                sys.exit(1)
        else:
            try:
                s3_conn.create_bucket(Bucket=S3_TEST_LEGACY_BUCKET_NAME)
        ...
        if s3_conn.Bucket(S3_TEST_PROD_BUCKET_NAME).creation_date is not None:
            try:
                s3_conn.Bucket(S3_TEST_PROD_BUCKET_NAME).objects.all().delete()
                s3_conn.Bucket(S3_TEST_PROD_BUCKET_NAME).delete()
                s3_conn.create_bucket(Bucket=S3_TEST_PROD_BUCKET_NAME)
            except Exception as e:
                logging.error(f"Error while creating testing buckets: {e}")
                sys.exit(1)
        else:
        ...
        # Fill in testing DB and testing legacy bucket with data
        for path in generate_path(50):
            try:
                s3_conn.Bucket(S3_TEST_LEGACY_BUCKET_NAME).put_object(Key=f"{path}", Body=DUMMY_AVATAR)
            except Exception as e:
                logging.error(f"Error while creating an s3 object: {e}")
                sys.exit(1)

            with db_conn.cursor() as curs:
                curs.execute("INSERT INTO avatars (path) VALUES (%s)", (path,))
                db_conn.commit()
    ...
    def test_fetch_data(self):
        ...
        fetch_data(db_conn,filename)
        self.assertTrue(os.path.exists(filename),"File does not exist")
        self.assertTrue(os.stat(filename).st_size != 0,"File is empty")
        db_conn.close()

    def test_copy_s3_objects(self):
        path=TEST_PATH
        try:
            s3_conn = boto3.resource('s3',
                    endpoint_url=S3_ENDPOINT_URL,
            ...
            logging.error(f"Error while connecting to S3: {e}")
            sys.exit(1)
        self.assertTrue(copy_s3_object(s3_conn, S3_TEST_LEGACY_BUCKET_NAME, S3_TEST_PROD_BUCKET_NAME, path),"S3 object have not been copied")
    ...
    def test_update_db(self):
        ...
        self.assertTrue(update_db_row(db_conn, path),"Path was not updated in DB")
    ...
    def test_delete_s3_object(self):
        ...
        try:
            ...
            logging.error(f"Error while connecting to S3: {e}")
            sys.exit(1)
        self.assertTrue(delete_s3_object(s3_conn, S3_TEST_LEGACY_BUCKET_NAME, path),"S3 object have not been deleted")            
    ...
    def tearDownClass(cls):
        ...
        try:
            ...
            logging.error(f"Error while deleting testing database: {e}")
            sys.exit(1)
        db_conn.close()

        # Delete testing S3 buckets
        try:
            s3_conn = boto3.resource('s3',
                    endpoint_url=S3_ENDPOINT_URL,
                    aws_access_key_id=AWS_ACCESS_KEY_ID,
        ...
        if s3_conn.Bucket(S3_TEST_LEGACY_BUCKET_NAME).creation_date is not None:
            try:
                ...
                logging.error(f"Error while deleting testing buckets: {e}")
                sys.exit(1)
